...
export function useRecepcionData() {
    ...
    const handleDniBlur = async (dni) => {
        ...
        try {
            ...
            const docType = /** @type {"DNI" | "RUC"} */ (form.tipo_documento);
    ...
    const actualizarEstado = useMutation({
        ...
        onError: (err, newVariables, context) => {
            if (context?.previousReservas) qc.setQueryData(['reservas', hotelId], context.previousReservas);
            if (context?.previousHabitaciones) qc.setQueryData(['habitaciones', hotelId], context.previousHabitaciones);
    ...
    const filtradas = useMemo(() => {
        const matchingReservations = reservas.filter(r => {
            if (!matchesReceptionFilter(r, /** @type {any} */ (filtro))) return false;
