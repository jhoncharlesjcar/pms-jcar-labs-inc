    ...
    return (
        <div ref={cardRef} className="enterprise-card section-card ui-card-pad col-span-2 flex flex-col transition-all duration-300 ease-out lg:col-span-2">
            <div className="flex items-center justify-between mb-4">
                ...
                <Link to="/ventas">
                    <span className="text-xs font-medium text-primary hover:text-primary/80 transition-colors flex items-center gap-1">Ver Historial <ArrowUpRight className="w-3 h-3" /></span>
                </Link>
            </div>
                    ...
                    return (
                        <div key={`${r._tipo}-${r.id}`} className="flex items-center justify-between p-3 bg-muted/20 rounded-xl hover:bg-muted/40 transition-colors group/item">
                            <div className="flex items-center gap-2.5">
                                <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 text-white text-xs font-bold bg-gradient-to-br shadow-sm", getAvatarColor(nombre))}>
                                    {initials}
                                </div>
                                <div>
                            ...
                            <div className="text-right">
                                ...
                                <p className="text-xs text-muted-foreground uppercase tracking-wider mt-0.5">{r.metodo_pago}</p>
                            </div>
                        </div>
                    );
                })}
