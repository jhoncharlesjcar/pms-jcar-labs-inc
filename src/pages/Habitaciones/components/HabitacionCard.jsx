        ...
        >
            ...
            <div className={cn("flex items-center justify-between gap-2", h.imagen_url ? "absolute top-2 left-2 right-2 z-10" : "")}>
                <div className="flex min-w-0 items-center gap-2">
                    ...
                    <StatusBadge status={h.estado} label={cfg.label} showIcon={false} className="rounded-full px-2 py-1 text-xs font-extrabold tracking-[0.08em]" />
                </div>
            ...
            <div className="my-4 flex items-end justify-between gap-4">
                <div className="min-w-0">
                    <p className="mb-1 text-xs font-extrabold uppercase tracking-[0.15em] text-muted-foreground">Habitación</p>
                ...
                <div className="flex-shrink-0 text-right">
                    <p className="text-xs font-bold uppercase tracking-[0.1em] text-muted-foreground">Tarifa noche</p>
            ...
            <div className="flex min-h-8 items-center justify-between gap-3 border-t border-border/60 pt-3">
                <div className="flex min-w-0 items-center gap-1.5 text-muted-foreground">
                    ...
                    }) : <span className="truncate text-xs font-semibold">Equipamiento estándar</span>}
                </div>
                <span className="flex-shrink-0 rounded-lg bg-muted/65 px-2 py-1 text-xs font-extrabold text-muted-foreground">{h.capacidad || 1} huésped{Number(h.capacidad || 1) === 1 ? '' : 'es'}</span>
            </div>
        </article>
    );
}
